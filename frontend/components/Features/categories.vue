...
<template>
    <div class="relative mt-10 cursor-pointer">
        ...
        <div class="relative w-full">
            <swiper ref="carousel" :slides-per-view=7 :space-between=10 :breakpoints="responsiveOptions"
                ...
                <swiper-slide v-for="(item, index) in categories" :key="index">
                    <NuxtLink :to="`/category/${item.id}`">
                        <div
                            class="flex flex-col items-center justify-center border mb-6 py-2 text-center rounded-sm hover:bg-primary hover:text-light">
                            <Icon v-if=item.icon :name="item.icon" class="mb-2 text-4xl" />
                            <span>{{ item.name }}</span>
                        </div>
                    </NuxtLink>
                </swiper-slide>
