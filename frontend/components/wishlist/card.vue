    ...
    <div class="relative">
        <div class="relative flex items-center w-full h-60 justify-center bg-gray-100 cursor-pointer group">
            <NuxtLink :to="`/product/${product.id}`">
                <div v-if="product.regular_price > 0" class="absolute px-2 py-1 text-xs font-bold rounded-md text-light bg-primary top-2 left-2">
            ...
            <div class="absolute flex flex-col space-y-2 right-4 top-2">
                <button class="p-2 text-black h-8 w-8 flex justify-center items-center bg-white rounded-full" @click="addToWishlist(product)">
                    <Icon :name="whishList ? 'iconamoon:trash-light' : 'fluent:heart-28-regular'" :class="['text-xl', { 'text-primary': whishList }]" />
                </button>
        ...
        <NuxtLink :to="`/product/${product.id}`">
            <div class="mt-4 text-left">
                ...
                <div class="flex items-center gap-4 mt-2">
                    ...
                    <div class="line-through text-secondary"><Currency :amount="product.regular_price"/></div>
                </div>
