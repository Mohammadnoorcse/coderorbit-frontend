...
onMounted(async () => {
  ...
  try {
    let data = await $fetch(`${EndPoint}/customer/dashboard?limit_per_page=10&status=7`,
			{
...
// console.log("Invoices Length:", invoices.value.length);
const paginate = async (pageNo) => {
    ...
    if (currentPage.value != pageNo && pageNo > 0 ) {
      ...
      isLoading.value = true;
        ...
        const data = await $fetch(`${EndPoint}/customer/dashboard?limit_per_page=10&page=${pageNo}&status=7`,
          {
            method: 'get',
            headers: headers.value,
              ...
              <tbody class="bg-white divide-y divide-gray-200">
                <tr class="odd:bg-white even:bg-gray-50 border-0" v-for="(invoice, index) in invoices" :key="index">
                  ...
                  <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                    ...
                    <NuxtLink :to="`/account/order/${invoice.id}`" class="text-blue-500 hover:underline">{{
                  ...
                  <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                    <!-- {{ invoice.action }} -->
                    <NuxtLink :to="`/account/order/${invoice.id}`" class="text-blue-500 hover:underline">view
                    </NuxtLink>
                  </td>
                </tr>
              </tbody>
